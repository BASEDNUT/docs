...
Sixth experiment in the nutUSD research series. [Experiment I](/v2/research/nutusd-testnet.md) verified the boundary on the rehearsal shape; [Experiment II](/v2/research/nutusd-adversarial.md) recorded the failure modes; [Experiment III](/v2/research/nutusd-liquidation-envelope.md) completed the seizure envelope; [Experiment IV](/v2/research/nutusd-liquidity-rates.md) measured liquidity and rates; [Experiment V](/v2/research/nutusd-vault-machinery.md) exercised the vault machinery. This experiment runs the exact production shape on testnet: an 8-decimal collateral token with a two-feed composed oracle — the cbBTC recipe — through market creation, the exact-maximum borrow, the boundary, and a crash liquidation. Every number is a measured on-chain value.
...
| PR2 | Does the composed two-feed oracle price exactly? | Yes — mockBTC/BTC × BTC/USD over USDC/USD, source-derived prediction == measured price |
...
The oracle is the production topology: the collateral price is the product of the base feeds divided by the quote feeds, each answer scaled by its feed decimals and the token decimals — not a single direct feed. The production mainnet recipe is this exact topology with real feeds: cbBTC/BTC × BTC/USD over USDC/USD.
...
The liquidation settled exactly across the full decimal mix: 8-decimal collateral quoted through a two-feed composed oracle into 6-decimal debt. Cross-decimal precision is a documented failure class in lending integrations; on the canonical deployment with this recipe it is exact.

## What the production recipe inherits

| Property | Status |
|---|---|
| 8-decimal collateral settlement | Measured exact — boundary, borrow, liquidation |
| Composed two-feed oracle | Measured exact — price, boundary, liquidation |
...
The production mainnet step swaps mock feeds for the real cbBTC/BTC, BTC/USD, and USDC/USD Chainlink feeds and mockBTC for cbBTC. The adapter, market parameters, and integration math are this experiment's.
